// v3.1 SMART - تصحيح بمسافة + 3 نقط + شريط بطارية/ساعة/رسائل + سلاسة
const keyboardEl=document.getElementById('keyboard'),suggestionBar=document.getElementById('suggestionBar'),langIndicator=document.getElementById('langIndicator'),versionText=document.getElementById('versionText'),miniToast=document.getElementById('miniToast'),numpadOverlay=document.getElementById('numpadOverlay'),numpadGrid=document.getElementById('numpadGrid'),footerEl=document.getElementById('footer'),batteryDisplay=document.getElementById('batteryDisplay'),timeDisplay=document.getElementById('timeDisplay'),messagesDisplay=document.getElementById('messagesDisplay');
...
const dictionaries={
  en:{
    ...
    corrections:{"teh":"the","adn":"and","becuase":"because","recieve":"receive","seperate":"separate","occured":"occurred","u":"you","ur":"your","r":"are","thiss":"this","thatt":"that","hthis":"this","hthat":"that","fouad":"Fouad"},
    bigrams:{"hello":"there","how":"are","thank":"you","good":"morning","i":"am","whats":"up","see":"you"}
  },
  ar:{
    words:["السلام","عليكم","مرحبا","كيف","حالك","انا","انت","هو","هي","نحن","هذا","هذه","ذلك","في","من","الى","على","عن","مع","بعد","قبل","كتاب","قلم","بيت","مدرسة","عمل","وقت","يوم","سنة","موبايل","كيبورد","مسافة","تراك","باد","اعدادات","لغة","ثيم","صوت","الوان","شكرا","حبيبي","تمام","ظبط","الله","محمد","مصر","عربي","فؤاد","صباح","الخير","مساء","حلو","جميل","بحبك","وحشتيني","عامل","ايه","اخبارك","كويس","الحمدلله","سريع","كبير","مستقر","سلس","محسن","ذكي","رائع"],
    corrections:{
      "هاذا":"هذا","هاذي":"هذه","هاذه":"هذه","ذالك":"ذلك","هاؤلاء":"هؤلاء",
      "انامعرش":"انا معرفش","انامعرفش":"انا معرفش","معرش":"معرفش",
      "احمد":"أحمد","انشاء":"إنشاء","ان":"أن","الي":"إلى","الا":"إلا","لاكن":"لكن","هاذا":"هذا","هاذا":"هذا",
      "شكرن":"شكرا","عفون":"عفوا","مرحب":"مرحبا","تمام":"تمام"
    },
    bigrams:{"السلام":"عليكم","صباح":"الخير","مساء":"الخير","كيف":"حالك","شكرا":"جزيلا","ان":"شاء","ما":"شاء","الحمد":"لله","الله":"اكبر","انا":"معرفش"}
  },
  ar_full:{
    ...
    corrections:{"هاذا":"هذا","هاذي":"هذه","ذالك":"ذلك","انامعرش":"انا معرفش","معرش":"معرفش"},
    bigrams:{"السلام":"عليكم","صباح":"الخير"}
  }
};
...
function getSmartSuggestions(word,sentence){
  ...
  // تصحيح مباشر بمسافة - أهم حاجة
  if(dict.corrections && dict.corrections[lower]){
    autoFix=dict.corrections[lower];
    smartSugs.push({text:autoFix,type:'auto',icon:'✓',isAuto:true});
  }
  // bigrams
  ...
  dict.words.forEach(w=>{ if(w.toLowerCase().startsWith(lower)&&w.toLowerCase()!==lower){ wordSugs.push({w,sc:0}); } });
  if(wordSugs.length<5){
    dict.words.forEach(w=>{ const d=levenshtein(lower,w.toLowerCase()); if(d<=2&&d>0) wordSugs.push({w,sc:d}); });
  }
...
function updateSuggestions(){
  ...
  let html='';
  // لو فيه تصحيح تلقائي بمسافة - اعرضه أول واحد مميز
  if(sugs.autoFix){
  ...
  suggestionBar.querySelectorAll('[data-s]').forEach(el=>{
    el.addEventListener('click',()=>{
      ...
      setTimeout(()=>{
        ...
        updateSuggestions();
        playSound();
        showToast(isAuto?`✓ ${currentWord} → ${sug}`:'✓ '+sug);
      },30);
    });
  });
}

// عند كتابة مسافة - تصحيح تلقائي بمسافة
function handleSpace(){
  ...
  if(dict.corrections && dict.corrections[lower]){
    ...
    setTimeout(()=>{
      commit(correction+' ');
      showToast(`✓ ${currentWord} → ${correction}`);
      currentWord='';
      sentenceBuffer+=' '+correction+' ';
    },20);
    return true;
  }
  return false;
...
function updateFooter(){
  try{
    ...
    // رسائل وهمية - يمكن ربطها لاحقاً بـ Notification
...
function updateUI(){
  ...
  versionText.textContent=`v3.1 SMART`;
...
function attachEvents(){
  ...
  keys.forEach(k=>{
    ...
    const handlePointerUp=(e)=>{
      ...
      if(key==='fn'){
        const n=Date.now();
        if(n-(window.lastFnTap||0)<350){ if(numpadEnabled){ numpadOverlay.classList.toggle('show'); } else { isFullTrackpad=!isFullTrackpad; const ov=document.getElementById('overlay'); if(isFullTrackpad) ov.classList.add('show'); else ov.classList.remove('show'); } }
        window.lastFnTap=n;
        return;
      }
...
document.getElementById('autocorrectToggle')?.addEventListener('click',function(){autocorrectEnabled=!autocorrectEnabled; this.textContent=`${autocorrectEnabled?'✅':'❌'} تصحيح بمسافة: ${autocorrectEnabled?'مفعل':'مطفي'}`; saveSet('autocorrect',autocorrectEnabled?'true':'false'); showToast('تصحيح '+(autocorrectEnabled?'مفعل':'مطفي'));});
...
renderKeyboard();
console.log('Rapoo v3.1 SMART - هاذا→هذا بمسافة, 3 dots only, battery/time/messages footer, smoother');
