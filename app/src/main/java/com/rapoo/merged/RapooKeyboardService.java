...
public class RapooKeyboardService extends InputMethodService {
    ...
    public View onCreateInputView(){
        int h=(int)(310*getResources().getDisplayMetrics().density);
        LinearLayout c=new LinearLayout(this); c.setOrientation(LinearLayout.VERTICAL); c.setBackgroundColor(Color.parseColor("#0f0f12"));
        webView=new WebView(this); webView.setBackgroundColor(Color.TRANSPARENT);
        WebSettings s=webView.getSettings(); s.setJavaScriptEnabled(true); s.setDomStorageEnabled(true); s.setAllowFileAccess(true); s.setAllowFileAccessFromFileURLs(true); s.setAllowUniversalAccessFromFileURLs(true); s.setLoadWithOverviewMode(true); s.setUseWideViewPort(true); s.setCacheMode(WebSettings.LOAD_NO_CACHE); s.setMixedContentMode(WebSettings.MIXED_CONTENT_ALWAYS_ALLOW);
    ...
    public class WebAppInterface{
        ...
        @JavascriptInterface public void deleteText(){
            ...
            try{
                ...
                if(et!=null && et.selectionStart!=et.selectionEnd){
                    // في تحديد - امسح التحديد كله
                    ic.commitText("",1);
                } else {
                    ic.deleteSurroundingText(1,0);
                }
            }catch(Exception e){ ic.deleteSurroundingText(1,0); }
        }
        @JavascriptInterface public void deleteN(int n){
            ...
            }catch(Exception e){ ic.deleteSurroundingText(n>0?n:1,0); }
        }
        @JavascriptInterface public void deleteWord(){
            InputConnection ic=getCurrentInputConnection(); if(ic==null) return;
            try{
                ExtractedTextRequest r=new ExtractedTextRequest(); r.token=0;
                ExtractedText et=ic.getExtractedText(r,0);
                if(et==null || et.text==null) { ic.deleteSurroundingText(10,0); return; }
                int pos=et.selectionStart;
                if(pos!=et.selectionEnd){ ic.commitText("",1); return; }
                String txt=et.text.toString();
                int start=pos-1;
                while(start>=0 && Character.isWhitespace(txt.charAt(start))) start--;
                while(start>=0 && !Character.isWhitespace(txt.charAt(start))) start--;
                int del=pos-start-1;
                if(del>0) ic.deleteSurroundingText(del,0); else ic.deleteSurroundingText(1,0);
            }catch(Exception e){ ic.deleteSurroundingText(10,0); }
        }
        @JavascriptInterface public void sendKey(int c){ InputConnection ic=getCurrentInputConnection(); if(ic==null) return; ic.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN,c)); ic.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_UP,c)); }
