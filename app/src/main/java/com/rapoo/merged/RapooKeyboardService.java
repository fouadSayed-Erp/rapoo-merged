...
public class RapooKeyboardService extends InputMethodService {
    ...
    public View onCreateInputView(){
        ...
        LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,heightPx);
        container.addView(webView,p);
        return container;
    }
    public class WebAppInterface{
        ...
        @JavascriptInterface public void deleteN(int n){ InputConnection ic=getCurrentInputConnection(); if(ic!=null && n>0) ic.deleteSurroundingText(n,0); }
        ...
        @JavascriptInterface public void sendKeyWithModifiers(int c,boolean s,boolean ctrl,boolean alt){ InputConnection ic=getCurrentInputConnection(); if(ic==null) return; int m=0; if(s) m|=KeyEvent.META_SHIFT_ON|KeyEvent.META_SHIFT_LEFT_ON; if(ctrl) m|=KeyEvent.META_CTRL_ON|KeyEvent.META_CTRL_LEFT_ON; if(alt) m|=KeyEvent.META_ALT_ON|KeyEvent.META_ALT_LEFT_ON; ic.sendKeyEvent(new KeyEvent(0,0,KeyEvent.ACTION_DOWN,c,0,m)); ic.sendKeyEvent(new KeyEvent(0,0,KeyEvent.ACTION_UP,c,0,m)); }
        @JavascriptInterface public void moveCursor(int dx){ try{ InputConnection ic=getCurrentInputConnection(); if(ic==null) return; ExtractedTextRequest r=new ExtractedTextRequest(); r.token=0; ExtractedText et=ic.getExtractedText(r,0); if(et==null) return; int pos=et.selectionStart+dx; if(pos<0) pos=0; if(pos>et.text.length()) pos=et.text.length(); ic.setSelection(pos,pos);}catch(Exception e){} }
